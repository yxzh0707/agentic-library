---
uuid: 019e29b8-bc96-722a-906e-33e030d7cf6f
node_type: raw
created_at: 2026-05-15T03:40:33.302Z
updated_at: 2026-05-15T03:59:26.447Z
created_by: human:default
created_by_run: import:pdf:Reformer_The_Efficient_Transformer.pdf
l0_summary: Reformer通过局部敏感哈希注意力和可逆残差层大幅提升Transformer效率。
l1_overview: Reformer引入两种关键技术解决Transformer在长序列上的高计算和内存成本：局部敏感哈希注意力将复杂度从O(L^2)降至O(L log L)，可逆残差层将激活存储从N倍减少到1倍。在长序列任务上，Reformer在保持性能的同时大幅提升速度和内存效率。
embeddings:
  e_l0_id: 159
  e_l1_id: 159
  e_l2_id: null
  model: BAAI/bge-m3
  embedded_at: 2026-05-15T03:59:17.758Z
wikilinks: []
current_path: /cluster_15
derived_state:
  cluster_id: 15
  cluster_membership_strength: 0.7185724768903692
...
hub_role:
  ...
  reason: 该节点讨论的内容与现有高效Transformer子话题方向相同，只是具体方法不同，应挂在已有的综述或子话题中心节点下。
  history:
    - changed_at: 2026-05-15T03:59:26.447Z
      from: neutral
      to: leaf
      changed_by: agent:librarian
      op_id: 019e29ca-06ea-722a-9071-712f5b01f98f
      reason: 该节点讨论的内容与现有高效Transformer子话题方向相同，只是具体方法不同，应挂在已有的综述或子话题中心节点下。
---
...
scaling by √dk.
For batching purposes we typically perform attention over a larger set ˜	Pi = {0, 1, . . . , l} ⊇ Pi
...
first ensure that h(kj ) = h(qj ) by setting kj = qj
...
LSH Attention max(bnhldk, bnhlnr (4l/nc)2) max(bnhldk, bnhnr l(4l/nc)2)
...
Published as a conference paper at ICLR 2020
Table 3: Memory and time complexity of Transformer variants. We write dmodel and df f for model
...
Transformer max(bldf f , bnhl2)nl (bldf f + bnhl2)nl
Reversible Transformer max(bldf f , bnhl2) (bnhldf f + bnhl2)nl
Chunked Reversible Transformer max(bldmodel, bnhl2) (bnhldf f + bnhl2)nl
LSH Transformer max(bldf f , bnhlnr c)nl (bldf f + bnhnr lc)nl
Reformer max(bldmodel, bnhlnr c) (bldf f + bnhnr lc)nl
4 RELATED WORK
