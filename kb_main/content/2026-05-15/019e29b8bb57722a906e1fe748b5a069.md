---
uuid: 019e29b8-bb57-722a-906e-1fe748b5a069
node_type: raw
created_at: 2026-05-15T03:40:32.983Z
updated_at: 2026-05-15T03:57:51.623Z
created_by: human:default
created_by_run: import:pdf:Perceiver_IO_General_Architecture_for_Structured_Inputs_and_Outputs.pdf
l0_summary: Perceiver IO：一种通用架构，可处理任意结构化输入和输出，线性复杂度。
l1_overview: Perceiver IO 是一种通用架构，通过灵活的输出查询机制处理任意结构化输入和输出，无需任务特定工程。它扩展了 Perceiver，能处理语言、视觉、光流、多模态及 StarCraft II 等任务，在 GLUE 上超越 BERT，在 Sintel 光流上达 SOTA，且输入输出规模线性扩展。
embeddings:
  e_l0_id: 156
  e_l1_id: 156
  e_l2_id: null
  model: BAAI/bge-m3
  embedded_at: 2026-05-15T03:57:41.311Z
wikilinks: []
current_path: null
derived_state:
  cluster_id: null
...
hub_role:
  ...
  reason: Perceiver IO 定义了一个新的子话题——通用架构中基于不对称注意力的方法，与现有邻居（如OneTrans、Gato等）相比，它提出了一种全新的处理任意结构化输入输出的范式，且是许多后续工作的基础，开辟了新的研究方向。
  history:
    - changed_at: 2026-05-15T03:57:51.623Z
      from: neutral
      to: center
      changed_by: agent:librarian
      op_id: 019e29c8-9482-722a-9071-436a339f92b0
      reason: Perceiver IO 定义了一个新的子话题——通用架构中基于不对称注意力的方法，与现有邻居（如OneTrans、Gato等）相比，它提出了一种全新的处理任意结构化输入输出的范式，且是许多后续工作的基础，开辟了新的研究方向。
---
...
i
arXiv:2107.14795v3 [cs.LG] 15 Mar 2022

-- 1 of 29 --

Published as a conference paper at ICLR 2022LÈ¡áyÒįŘįºμòμÒ¡y®
èèè#Ď ±gÅĎ gĎ ±»±{·Ď Ď F»{|·g Ď ·g·Ď
...
± {Ď Ď·{Ď g±ªgĎK{gĎg·ĎwgÅëĎ#Ď
±gÅĎ·{Ďw{sg·{Ďr {Ď±·»s·»{Ď Ď
gĎgwèèè
SμÒ¡´μÒĐ 	#Èy´´yÒ¡y®Đ
LyÈyÅ ÈyÌĐ 	μÒy¡®´μÒĐ 	5y®ċįÈÖ´´¡μ
ĉĉĉ
...
Text 	Byte-level pred. 	Embed. 	None 	2, 048 × 768 	2, 048 × 768
Text 	Multi-task (8 tasks) 	Embed. 	None 	2, 048 × 768 	8 × 768
Video 	Flow prediction 	None 	None 	365, 056 × 64 182, 528 × 64
Video 	Flow prediction 	Concat 	None 	182, 528 × 64 182, 528 × 64
Video 	Flow prediction 	Conv+maxpool 	RAFT upsampling 22, 816 × 64 	11, 408 × 64
Video 	Flow prediction 	Conv+maxpool+concat 	RAFT upsampling 11, 408 × 64 	11, 408 × 64
Video+Audio+Label 	Autoencoding 	Patch: 1x4x4 Vid, 16 Aud 	None 	50, 657 × 704 803, 297 × 512
Image 	Classification 	None 	None 	50, 176 × 3 	1 × 1, 000
Image 	Classification 	Linear projection 	None 	50, 176 × 256 	1 × 1, 000
Image 	Classification 	Conv+maxpool 	None 	3, 136 × 64 	1 × 1, 000
StarCraft Unit Set Encoding and Classification 	Tokenization 	Pointer network 	512 × 256 	512 × 128
Video+Audio 	Classification 	Patch: 2 × 8 × 8 Vid, 128 Aud 	None 	13, 024 × 487 	1 × 527
Video+Audio 	Classification 	Patch: 2 × 8 × 8 Vid. Aud → mel-spectrogram 	None 	17, 344 × 487 	1 × 527
...
along the first dimension), producing a final array XM LP ∈ RN ×D .
...
XQKV = Attn(layerNorm(XQ), layerNorm(XKV )) (4)
XQKV = XQKV + XQ (5)
XQKV = XQKV + MLP(layerNorm(XQKV )), (6)
...
complexity of O(M N F ). Let M , N , and O be the index dimensions for the input, latent, and output
...
latent attention blocks has complexity O([M + O + LN ]N F ). In other words, Perceiver IO has
