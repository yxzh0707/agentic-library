---
uuid: 019e29b8-b334-722a-906d-593506dde9c9
node_type: raw
created_at: 2026-05-15T03:40:30.900Z
updated_at: 2026-05-15T03:48:19.772Z
created_by: human:default
created_by_run: import:pdf:Deep_Neural_Networks_for_YouTube_Recommendations.pdf
l0_summary: 提出图卷积网络用于半监督节点分类，高效可扩展。
l1_overview: 提出图卷积网络（GCN）用于图结构数据的半监督分类。通过一阶近似谱图卷积，模型直接在图上操作，计算复杂度与边数成线性，学习整合局部图结构和节点特征的表示。在引文网络和知识图谱数据集上优于现有方法。
embeddings:
  e_l0_id: 132
  e_l1_id: 132
  e_l2_id: null
  model: BAAI/bge-m3
  embedded_at: 2026-05-15T03:48:11.364Z
wikilinks: []
current_path: /cluster_15
derived_state:
  cluster_id: 15
  cluster_membership_strength: 0.661018046846441
...
hub_role:
  ...
  reason: 该节点定义了图卷积网络在半监督节点分类中的新子话题，与现有推荐系统相关节点无明显关联，开辟了新的研究方向。
  history:
    - changed_at: 2026-05-15T03:48:19.772Z
      from: neutral
      to: center
      changed_by: agent:librarian
      op_id: 019e29bf-dab9-722a-906f-c4f88717f52e
      reason: 该节点定义了图卷积网络在半监督节点分类中的新子话题，与现有推荐系统相关节点无明显关联，开辟了新的研究方向。
---
...
IN is the identity matrix, ˜	Dii = ∑
j ˜	Aij and W (l) is a layer-specific trainable weight matrix. σ(·)
...
2 AD− 1
2 =
U ΛU >, with a diagonal matrix of its eigenvalues Λ and U >x being the graph Fourier transform
...
kTk( ˜L)x , (5)
with ˜L = 2
λmax L − IN ; as can easily be verified by noticing that (U ΛU >)k = U ΛkU >. Note that
...
0 and θ′
1. The filter parameters can be shared over the whole graph.
...
matrix. This filtering operation has complexity O(|E|F C), as ˜	AX can be efficiently implemented
...
with a symmetric adjacency matrix A (binary or weighted). We first calculate ˆ	A = ˜	D− 1
2 ˜	A ˜	D− 1
2 in
...
(dropout rate for all layers, L2 regularization factor for the first GCN layer and number of hidden
...
Chebyshev filter (Eq. 5) K = 3 ∑K
k=0 Tk( ˜L)XΘk
69.8 79.5 74.4
K = 2 69.6 81.2 73.8
1st-order model (Eq. 6) XΘ0 + D− 1
2 AD− 1
2 XΘ1 68.3 80.0 77.5
Single parameter (Eq. 7) (IN + D− 1
2 AD− 1
2 )XΘ 69.3 79.2 77.4
Renormalization trick (Eq. 8) ˜	D− 1
2 ˜	A ˜	D− 1
2 XΘ 70.3 81.5 79.0
1st-order term only D− 1
2 AD− 1
2 XΘ 68.7 80.5 77.8
...
˜	A = A + λIN . (11)
4Hardware used: 16-core Intel R© Xeon R© CPU E5-2640 v3 @ 2.60GHz, GeForce R© GTX TITAN X
8
...
Here, h(t)
i denotes the coloring (label assignment) of node vi (at iteration t) and Ni is its set of
...
take h(l)
i now to be a vector of activations of node i in the lth neural network layer. W (l) is a
layer-specific weight matrix and σ(·) denotes a differentiable, non-linear activation function.
By choosing cij = √didj , where di = |Ni| denotes the degree of node vi, we recover the propaga-
...
(2010). ˆ	A, X and Z are defined as in Section 3.1.
