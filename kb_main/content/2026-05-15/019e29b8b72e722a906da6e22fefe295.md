---
uuid: 019e29b8-b72e-722a-906d-a6e22fefe295
node_type: raw
created_at: 2026-05-15T03:40:31.918Z
updated_at: 2026-05-15T03:51:57.932Z
created_by: human:default
created_by_run: import:pdf:HSTU_Actions_Speak_Louder_than_Words_Generative_Recommendations.pdf
l0_summary: 提出HSTU架构，将推荐建模为生成式序列转导，在万亿参数规模上显著提升性能。
l1_overview: 本文提出HSTU（层次化序列转导单元）架构，将推荐问题重新形式化为生成式建模框架下的序列转导任务。该架构专为高基数、非平稳的流式推荐数据设计，在合成和公共数据集上NDCG提升高达65.8%，速度比基于FlashAttention2的Transformer快5.3-15.2倍。基于HSTU的生成式推荐模型参数达1.5万亿，在线A/B测试指标提升12.4%，已部署于拥有数十亿用户的大型平台。更重要的是，模型质量随训练计算量呈幂律扩展，为推荐领域的基础模型奠定了基础。
embeddings:
  e_l0_id: 141
  e_l1_id: 141
  e_l2_id: null
  model: BAAI/bge-m3
  embedded_at: 2026-05-15T03:51:45.520Z
wikilinks: []
current_path: /cluster_15
derived_state:
  cluster_id: 15
  cluster_membership_strength: 0.7508095030056925
...
hub_role:
  ...
  reason: 该节点是HSTU架构的原始论文，定义了推荐系统生成式建模这一子话题，且已有邻居（笔记）是其解释性内容，因此作为子话题的锚点。
  history:
    - changed_at: 2026-05-15T03:51:57.932Z
      from: neutral
      to: center
      changed_by: agent:librarian
      op_id: 019e29c3-2ee6-722a-9070-53137fcb3af5
      reason: 该节点是HSTU架构的原始论文，定义了推荐系统生成式建模这一子话题，且已有邻居（笔记）是其解释性内容，因此作为子话题的锚点。
---
...
Xuan Cao 1 Leon Gao 1 Zhaojie Gong 1 Fangda Gu 1 Michael He 1 Yinghai Lu 1 Yu Shi 1
Abstract
...
1. Introduction
Recommendation systems, quintessential in the realm of on-
...
Actions Speak Louder than Words: Trillion-Parameter Sequential Transducers for Generative Recommendations
Sequentialized+model
-based unified features
Sequentialized+model
-based unified features
Sequentialized+model
-based unified features
...
is the user’s representation at token i. A typical objective is
...
scales as P
i ni(n2
i d + nidf f d), where ni is the number of
...
the overall time complexity reduces to O(N 3d + N 2d2),
...
specifically, when we sample the i-th user at rate su(ni), the
total training cost now scales as P
i su(ni)ni(n2
i d + nid2),
which is reduced to O(N 2d + N d2) by setting su(ni) to
1/ni. One way to implement this sampling in industrial-
scale systems is to emit training examples at the end of a
user’s request or session, resulting in ˆ	su(ni) ∝ 1/ni.
...
Q(X), keys K(X), values V (X), and gating weights U (X)
...
ate states of 2d + 4df f + 2d + 1d = 4d + 4df f . Here,
...
tive to n. We optionally divide the overall m candidates into
...
a sequence of length 4096 is turned into a sequence of length
...
5. Related Work
Prior work on sequential recommenders reduces user interac-
...
6. Conclusions
We have proposed Generative Recommenders (GRs), a new
...
ume 27, 2014.
Sileo, D., Vossen, W., and Raymaekers, R. Zero-shot rec-
...
• Feature-Weighted Selection – Selects L items from S according to a weighted distribution 1 − fn,i/(PL
j=1 fj,i)
...
...	Φ'0 	Φ'bm-1
X
Φ'0 	Φ'bm-1 Φ'bm-1 	... 	Φ'2bm-1 	... 	Φ'm-1
Microbatch 0 	Microbatch 1 	Microbatch m/bm-1
All m Ranking
Candidates
Microbatches
in M-FALCON
...
with m candidates Φ′
0, . . . , Φ′
m−1, divided into ⌈m/bm⌉ microbatches, where we show model inference for the first microbatch
Φ′
0, . . . , Φ′
bm−1 (with 2nc + bm total tokens after Φ0, a0, . . . , Φnc−1, anc−1 are taken into account) above the dotted line. Note that the
self-attention algorithm is modified such that Φ′
i cannot attend to Φ′
j when i̸ = j – this is highlighted with “×” in the figure.
...
inference for one item at a time, with a cost of O(mn2d) for m candidates and a sequence length of n. Here we show that
...
batch such operations (“batched inference”) and reduce cost to O((n + m)2d) = O(n2d).
...
i, j ≥ 2nc, i̸ = j to False or −∞ to prevent target positions Φ′
0, . . . , Φ′
bm−1 from attending to each other. It is easy to see
that by doing so, the output of the self-attention block for Φ′
i, a′
i, only depends on Φ0, a0, . . . , Φnc−1, anc−1, but not on Φ′
j
(i̸ = j). In other words, by making a forward pass over (2nc + bm) tokens using the modified attention mask, we can now
obtain the same results for the last bm tokens as if we’ve made bm separate forward passes over (2nc + 1) tokens, with Φ′
i
...
microbatches of size bm such that O(bm) = O(n), which retains the O((n + m)2d) = O(n2d) running time previously
...
for the bm candidates. This reduces the cached forward pass’s computational complexity to O(bmd2 + bmnd), which
significantly improves upon O((n + bm)d2 + (n + bm)2d) by a factor of 2-4 even when bm = n.
Algorithm 1 M-FALCON Algorithm.
1: Input: Merged token series x0, x1, . . . , xn−1 (can be e.g., (Φ0, a0, . . . , Φnc−1, anc−1) where n = 2nc); m ranking
candidates Φ′
0, . . . , Φ′
m−1; a b-layer h-heads self-attention model trained in causal autoregressive settings (e.g., HSTU or
...
2: Output: Predictions for all m ranking candidates, (a′
0, . . . , a′
m−1).
...
5: attnM ask[i, j] = −∞ for i, j ≥ n, i̸ = j {This prevents the last bm entries from attending to each other.}
6: (a′
0, a′
1, . . . , a′
bm−1), kvCache ← f (embLayer((x0, x1, . . . , xn−1, Φ′
0, . . . , Φ′
bm−1)), ∅, attnM ask)
7: predictions = (a′
0, a′
...
bmi, a′
bmi+1, a′
bm(i+1)−1), ← f (embLayer((x0, x1, . . . , xn−1, Φ′
bmi, . . . , Φ′
bm(i+1)−1)), kvCache, attnM ask)
11: predictions ← predictions + (a′
bmi, a′
bmi+1, . . . , a′
bm(i+1)−1)
12: i ← i + 1
