---
uuid: 019e29b8-bc08-722a-906e-24951e650ea9
node_type: raw
created_at: 2026-05-15T03:40:33.160Z
updated_at: 2026-05-15T03:58:24.051Z
created_by: human:default
created_by_run: import:pdf:Performer_Rethinking_Attention_with_Performers.pdf
l0_summary: Performers通过FAVOR+方法以线性复杂度近似softmax注意力，保持准确性和理论保证。
l1_overview: 本文提出Performers，一种线性时间和空间复杂度的Transformer架构，能够以可证明的准确度估计标准的softmax全秩注意力，无需稀疏或低秩先验。其核心是FAVOR+方法，通过正交随机特征近似注意力核，支持无偏或近无偏估计、均匀收敛和低方差。Performers与常规Transformer完全兼容，并在像素预测、文本建模、蛋白质序列等多任务上取得竞争性结果，同时可推广至其他可核化注意力机制，为首次大规模比较softmax与其他核提供可能。
embeddings:
  e_l0_id: 157
  e_l1_id: 157
  e_l2_id: null
  model: BAAI/bge-m3
  embedded_at: 2026-05-15T03:58:15.805Z
wikilinks: []
current_path: /cluster_15
derived_state:
  cluster_id: 15
  cluster_membership_strength: 0.7354261513284467
...
hub_role:
  ...
  reason: 该论文属于高效注意力机制子方向，已有线性Transformer和Linformer作为中心节点，Performer是其中一种具体方法，未开辟全新子话题。
  history:
    - changed_at: 2026-05-15T03:58:24.051Z
      from: neutral
      to: leaf
      changed_by: agent:librarian
      op_id: 019e29c9-132f-722a-9071-563b62baa0f1
      reason: 该论文属于高效注意力机制子方向，已有线性Transformer和Linformer作为中心节点，Performer是其中一种具体方法，未开辟全新子话题。
---
...
Att↔(Q, K, V) = D−1AV, A = exp(QK>/√d), D = diag(A1L). (1)
...
defined for the (usually randomized) mapping: φ : Rd → Rr
+ (for some r > 0) as:
K(x, y) = E[φ(x)>φ(y)]. (3)
We call φ(u) a random feature map for u ∈ Rd. For Q′, K′ ∈ RL×r with rows given as φ(q>
i )>
and φ(k>
i )> respectively, Equation 3 leads directly to the efficient attention mechanism of the form:
̂
Att↔(Q, K, V) =̂ D−1(Q′((K′)>V)),	̂	 D = diag(Q′((K′)>1L)). (4)
Herê Att↔ stands for the approximate attention and brackets indicate the order of computations. It is
...
2 ), l = 2, f1 = sin, f2 = cos. We call it̂ SMtrig
m (x, y).
...
̂
SMtrig
m and provide detailed theoretical explanations showing that the variance of̂ SMtrig
m is large
...
where Λ = exp(− ‖x‖2+‖y‖2
2 ) and cosh is hyperbolic cosine. Consequently, softmax-kernel admits a
positive random feature map unbiased approximation with h(x) = exp(− ‖x‖2
2 ), l = 1, f1 = exp
and D = N (0, Id) or: h(x) = 1	√2 exp(− ‖x‖2
2 ), l = 2, f1(u) = exp(u), f2(u) = exp(−u) and the
same D (the latter for further variance reduction). We call related estimators:̂ SM+
m and̂ SMhyp+
m .
...
positive features to efficiently train softmax-based linear Transformers. If we replace in (7) ω with	√d ω
‖ω‖ , we obtain the so-called regularized softmax-kernel SMREG which we can approximate in
a similar manner, simply changing D = N (0, Id) to D = Unif(√dSd−1), a distribution correspond-
ing to Haar measure on the sphere of radius √d in Rd, obtaining estimator	̂	 SMREG+
m. As we show
...
particularly well with our introduced PRFs for softmax. This leads to the first theoretical results
...
methods, see: next section) and leads to the first exponentially small bounds on large deviations
...
which results in an accurate as well as space and time efficient mechanism which we call FAVOR+.
...
for the softmax-kernel satisfies: ‖A‖∞ ≤ C for some constant C ≥ 1. Denote by Areg the
...
Areg(i, j)
A(i, j) ≤ 1. (9)
Furthermore, the latter holds for d ≥ 2 even if the L∞-norm condition is not satisfied, i.e. the
...
Theorem 2. If̂ SMort+
m (x, y) stands for the modification of̂ SM+
...
P[̂SMREG+
m(x, y) > a] ≤ exp(−θma)MZ (θ)m, P[̂SMREGort+
m (x, y) > a]
≤ exp(−θma)
(
MZ (θ)m − exp
...
where	̂	 SMREGort+
m (x, y) stands for the modification of	̂	 SMREG+
m(x, y) with ORFs, X =
Λ exp(√d ω>
...
queries/keys are upper-bounded by R > 0. Define l = Rd− 1
4 and take h∗ = exp( l2
2 ). Then
for any  > 0, δ = 
...
4 R
δ )) the fol-
lowing holds for the attention approximation mechanism leveraging estimatorŝ SMtrig
with ORFs:
‖̂A − A‖∞ ≤  with any constant probability, wherê A approximates the attention matrix A.
4 EXPERIMENTS
...
et al., 2020). In Fig. 6, the Reformer and Linformer significantly drop in accuracy on the protein
...
Transformer and confirming our theoretical claims from Section 3.
...
is specified in the Generalized Attention (Subsec. A.4), and unless specifically mentioned (e.g.
...
fixed vocabulary size, instead opting for any tokenization which can model an arbitrary string of
...
Unless specifically stated, all Performer + Transformer runs by default used 0.5 grad clip, 0.1 weight
decay, 0.1 dropout, 10−3 fixed learning rate with Adam hyperparameters (β1 = 0.9, β2 = 0.98,  =
10−9), with batch size maximized (until TPU memory overload) for a specific model.
...
Result:̂ Att↔(Q, K, V) ∈ RL×L if isBidirectional,̂ Att→(Q, K, V) ∈ RL×L otherwise.
Compute Q′ and K′ as described in Section 2.2 and Section 2.3 and take C := [V 1L];
...
:,l,p is therefore a result of a prefix-sum
(or cumulative-sum) operation applied to G:,l,p: GPS
i,l,p = ∑i
j=1 Gi,l,p. An efficient algorithm to
...
2020), we argue that it includes most of the relevant sequences. Specifically, the TrEMBL dataset
...
length L = 1024, which results in few sequences being truncated significantly.
...
on each original (Q, K, V ) triple to obtain the attention output, and once on a modified (Q, K, V ◦)
...
proteins, which contains 100 amino acids. In Figure 10, we show the attention matrices for the first
4 layers. Note that many heads show a diagonal pattern, where each node attends to its neighbors,
and some heads show a vertical pattern, where each head attends to the same fixed positions. These
...
That completes the proof of the first part of the lemma. An identity involving hyperbolic cosine
...
(15)
which completes the first part of the proof. To obtain the formula for: MSE(̂SM+
m(x, y)) notice first
that:
...
SM(x, y) = exp(− ‖x‖2 + ‖y‖2
2 )Eω∼N (0,Id)[exp(ω>z)]. (17)
Therefore we obtain:
MSE(̂SM+
m(x, y)) = 1
m exp(−(‖x‖2 + ‖y‖2))Var(exp(ω>z)) =
1
m exp(−(‖x‖2 + ‖y‖2)) (E[exp(2ω>z)] − (E[exp(ω>z)])2) =
1
...
m (x, y)) = 1
4m exp(− ‖x‖2 + ‖y‖2
2 )2(Var(exp(ω>z)) + Var(exp(−ω>z))+
2Cov(exp(ω>z)), exp(−ω>z)))) = 1
4m exp(− ‖x‖2 + ‖y‖2
2 )2(2Var(exp(ω>z))+
2Cov(exp(ω>z)), exp(−ω>z))))) = 1
2m exp(−(‖x‖2 + ‖y‖2))
(Var(exp(ω>z)) + 1 − (E[exp(ω>z)])2) = 1
2m exp(−(‖x‖2 + ‖y‖2))
(exp(2‖z‖2) − exp(‖z‖2) + 1 − exp(‖z‖2)) = 1
...
Proof. Let x, y ∈ Rd be respectively a query/key. Note that from the definition of SMREG(x, y)
we have for z = x + y:
...
Published as a conference paper at ICLR 2021
Lemma 3. Expression A(2k, d) satisfies the following for k ∈ N :
A(2k, d) = (2k − 1)!!
(d + 2k − 2)(d + 2k − 4) · ... · d . (23)
...
wk
k! f (k, d). (30)
Note first that for k ≥ 1 we have: f (k, d) ≤ 1, thus:
SMREG(x, y) ≤ SM(x, y). (31)
30
...
Definition 1. We say that function F : Rn → R is beautiful if F can be expressed as:
FΩ,g (z) = Eω∼Ω[g(ω>z)], (36)
for a probabilistic isotropic distribution Ω, and where g : R → R is an entire function with non-
negative power-series coefficients (i.e. g(x) = ∑∞
...
2 )FΩreg ,g (z), where Ωreg stands for the distribution
...
i ∼ Ω, but (ωort
i )>ωort
j = 0 for i 6 = j (such an orthogonal ensemble can be always created
...
Denote by MZ (θ) = E[eθZ ] a moment generating function of the random variable Z. Note first
...
where we have: Xort
i = g((ωort
i )>z). We see that it suffices to show that for any θ > 0 the following
holds: E[eθ(Xort
1 +...+Xort
m )] < E[eθ(Xiid
1 +...+Xiid
...
m )jm ])
(47)
Note first that using the fact that f is entire, we can rewrite each Xort
i as:
Xort
i =
∞	∑
...
i and Xiid
i int the formula for ∆ and expanding power-
...
1 )>z)d1 · ... · ((ωort
m )>z)dm ]. (51)
Our next goal is to re-write the formula for̂ ∆(d1, ..., dm). Denote:
Y = ((ωort
1 )>z)d1 · ... · ((ωort
m )>z)dm . (52)
33
...
1 ‖2, ..., ‖ωort
m ‖2.
This comes from the fact that for a fixed z one can think about the set: ωort
1
‖ωort
	1 ‖2 , ..., ωort
m
...
vector z has the same distribution as: g
‖g‖2 ‖z‖2.
...
d
d1+...+dm ] (55)
Now let us focus on the second expression from the formula on̂ ∆(d1, ..., dm). We have:
E[((ωiid
1 )>z)d1 · ... · ((ωiid
m )>z)dm ] =
m	∏
...
di ],
(56)
where the first equality comes from the fact that different ωiid
...
k1+...+ks ] · E[‖˜g‖k1+...+ks
2 ], (58)
where the first equality comes from the independence of different elements of z = (z1, ..., zn)> and
...
Published as a conference paper at ICLR 2021
Note that by Lemma 5, we can rewrite the right expression from the formula on̂ ∆(d1, ..., dm) as:
E[(‖ωort
1 ‖2)d1 ] · ... · E[(‖ωort
m ‖2)dm ] · ‖z‖d1+...+dm
2
...
. (60)
The left expression from the formula on̂ ∆(d1, ..., dm) can be rewritten as:
L(d1, ..., dm) = E[(‖ωiid
1 ‖2)d1 ] · ... · E[(‖ωiid
m ‖2)dm ] · ‖z‖d1+...+dm
2
...
i and ωiid
i are the same, we can rewritê ∆(d1, ..., dn) as:
̂
∆(d1, ..., dm) = L(d1, ..., dm)(1 − τ (d1, ..., dm)), (62)
...
(63)
We need now few observations regardinĝ ∆(d1, ..., dm). Note firsr that since odd moments of
the Gaussian scalar distribution N (0, 1) are zero,̂ ∆(d1, ..., dm) is zero if at least of of di is odd.
Furthermore,	̂	 ∆(d1, ..., dm) is trivially zero if all but at most one di are zero.
...
cj1,...,jm (d1, . . . , dm)
×L(d1, ..., dm).
Therefore (see: our observations on̂ ∆(d1, ..., dm)) to complete the proof it suffices to show that:
τ (d1, ..., dm) ≤ d
d+2 if at least two: di, dj for i 6 = j are nonzero and all di are even.
...
μd(∑m
i=1 di) , (65)
where μd(j) stands for the jth moment of the χ-distribution with d degrees of freedom. Note that
μd(j) = 2 j
2 Γ( d+j
2 )
Γ( d
...
for a fixed d, the RHS of the Equality 65 is maximized when di = dj = 2 and dk = 0 for some i 6 = j
and k /∈ {i, j}. Furthermore, straightforward calculations show that in that case the value of the RHS
from Equality 65 is d
...
for some i 6 = j, di, dj > 0 and all di are even. As we have shown above,̂ ∆(d1, . . . , dm) = 0 when
...
we take j = 4, a subset of S4 with only two nonzero jk1 = jk2 = 2 for some k1 6 = k2 (there are	(m
2
...
a2
1am−2
0 . For d1, . . . , dm in such set, we’ll have τ (d1, . . . , dm) ≤ d
...
Proof. We will use the notation from the proof of Theorem 5. Since both estimators:̂ F ort
m (z) and̂
F iid
m (z) are unbiased, we have: MSE(̂F ort
m (z)) = Var(̂ F ort
m (z)) and MSE(̂F iid
m (z)) = Var(̂ F iid
m (z)).
...
i and Xiid
i from Equation 48 and Equation 49, and using our analysis
...
Theorem 7 (Uniform convergence for the trigonometric mechanism). Define entries of the attention
...
σ = Eω∼Ω[ω>ω] the following holds: ‖̂ A − A‖∞ ≤  with any constant probability, wherê A
...
4
k>
j )]i,j ∈ RL×L. Denote bŷ A and
approximation of the attention matrix obtained from trigonometric orthogonal random features and
bŷ B an approximation of matrix B that those random features provide. We rely on Theorem 3 from
...
4
. We have:
‖̂A − A‖∞ = ‖DQ(̂B − B)DK‖∞ ≤ ‖DQ‖∞‖̂B − B‖∞‖DK‖∞ ≤ δg∗h∗ (77)
Taking δ = 
...
dependence on R means that the length of queries and keys cannot grow at a fixed m if we want to
retain the quality of the approximation. In particular, this means that FAVOR cannot approximate
hard attention on sequences of unlimited length with a fixed m. When the sequence length increases,
