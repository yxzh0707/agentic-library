...
具体判断信号:
- "讨论同一个模型的不同方面(结构、训练、推理)" → 同一个 working set
- "讨论同一个比赛的不同题号/不同方案" → 同一个 working set
- "讨论同一个研究方向的不同论文/方法" → 同一个 working set
- "一篇讲前端 React,一篇讲后端数据库,但两者是同一个产品" → 同一个 working set
- "一篇讲推荐系统,一篇讲公司法" → 两个 working set

判断时看文档的**实质语境和共同目标**,不是看文档的**标题措辞或技术栈差异**。

反例警示:
如果 ${N} 篇文档全是某个比赛/课程的衍生内容,正确答案是 1 个 working set。
错误做法:拆成"赛题策略""模型优化""工程落地"等多个 working set。
...

{
  "working_sets": [
    {
      "label": "<2-8 汉字的项目/语境名,例如 'TAAC 2026 推荐赛题' 或 '酸面团发酵研究'>",
